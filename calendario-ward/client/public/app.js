...
// veces las ayudas son esas 3 categorías juntas") y varias vistas necesitan
...
// Categorías de un caso, ya con el fallback ['otro'] para casos viejos que
// por algún motivo llegaran sin ninguna (nunca debería pasar tras la
// migración del servidor, pero more robusto no asumirlo en el cliente).
function welfareCaseCategories(c) {
  return (c.categories && c.categories.length) ? c.categories : ['otro'];
}
function welfareCategoriesLabel(c) {
  return welfareCaseCategories(c).map((cat) => WELFARE_CATEGORY_LABELS[cat] || cat).join(', ');
}

// Checklist de cierre (Punto pedido explícitamente: "antes de marcarlo
// cerrado, pedir confirmar 2-3 puntos, por ejemplo si se devolvió el
// formulario firmado, se informó a la persona"). Las claves deben calzar
// 1:1 con WELFARE_CLOSURE_CHECKLIST_KEYS del servidor (routes/welfare.js) —
// el servidor es quien de verdad lo exige; esto solo arma la UI.
const WELFARE_CLOSURE_CHECKLIST = [
  { key: 'formSigned', label: 'El formulario de autosuficiencia fue completado (y adjuntado aquí, si corresponde)' },
  { key: 'personNotified', label: 'Se informó a la persona o familia que el caso se da por cerrado' },
  { key: 'noFurtherNeed', label: 'No queda ninguna necesidad pendiente sin registrar' },
];
// `idPrefix` evita colisión de ids cuando coexisten dos formularios con
// checklist en la misma página (no pasa hoy, pero es gratis prevenirlo).
// Arranca oculto y sin `required` — quien la muestra (el select de estado o
// de decisión) es quien activa/desactiva `required` junto con la
// visibilidad, para que un checkbox escondido nunca bloquee un envío que no
// tiene nada que ver con cerrar el caso.
function welfareClosureChecklistHtml(idPrefix) {
  return `
    <div id="${idPrefix}-checklist" class="hint-box" style="display:none; margin:10px 0;">
      <div style="font-weight:600; margin-bottom:6px; font-size:12.5px;">✅ Antes de cerrar, confirma:</div>
      ${WELFARE_CLOSURE_CHECKLIST.map((it) => `
        <label style="display:flex; gap:6px; align-items:flex-start; font-size:12.5px; margin-bottom:5px; cursor:pointer;">
          <input type="checkbox" name="closureChecklist.${it.key}" style="margin-top:2px;" />
          <span>${esc(it.label)}</span>
        </label>`).join('')}
    </div>`;
}
// Muestra/oculta el bloque de checklist y activa/desactiva `required` en
// sus checkboxes a la vez — si se esconde, además los desmarca, para que
// alternar la decisión/estado de un lado a otro nunca deje un "sí, confirmo"
// colgado de una vista anterior sin que la persona lo haya visto de nuevo.
function toggleWelfareChecklist(idPrefix, show) {
  const block = document.getElementById(`${idPrefix}-checklist`);
  if (!block) return;
...
// Arma el objeto `closureChecklist` a partir del <form> ya enviado — se usa
// solo cuando corresponde adjuntarlo al body (ver los 3 lugares que cierran
// un caso: editar caso, evaluación mensual, revisión rápida desde la
// tarjeta).
function collectWelfareClosureChecklist(form) {
  const fd = new FormData(form);
  const out = {};
  WELFARE_CLOSURE_CHECKLIST.forEach((it) => { out[it.key] = fd.get(`closureChecklist.${it.key}`) === 'on'; });
  return out;
}
    ...
    parts.push(`
      <form id="wfd-review-form">
        <div class="field"><label>¿Cómo sigue esta persona/familia?</label>
          <select name="decision" id="wfd-review-decision">
        ...
        ${welfareClosureChecklistHtml('wfd-review')}
...
// Búsqueda por nombre (Punto pedido explícitamente: "no hay un buscador de
// nombre, tengo que buscar de forma difícil") — también matchea descripción
// y categoría, sin distinguir mayúsculas/tildes (normalizeSearchText, mismo
// criterio que ya usa el buscador de Usuarios).
function welfareMatchesSearch(c, search) {
  if (!search) return true;
  const haystack = [c.memberName, c.description, welfareCategoriesLabel(c)].join(' ');
  return normalizeSearchText(haystack).includes(search);
}
// Filtro por categoría (Punto pedido explícitamente: "que se pueda
// seleccionar más de 1") — arreglo vacío = sin filtro (se muestran todas);
// con 1 o más categorías marcadas, se muestra un caso si tiene AL MENOS UNA
// de ellas (no exige que las tenga todas).
function welfareMatchesCategoryFilter(c, filter) {
  if (!filter || !filter.length) return true;
  return welfareCaseCategories(c).some((cat) => filter.includes(cat));
}
// Orden por urgencia (Punto pedido explícitamente): primero los casos con
// evaluación vencida, después los que vencen pronto (los más próximos
// primero), después los que no tienen evaluación pendiente (por última
// actualización, más reciente primero) — los cerrados siempre al final
// (relevante en la sub-pestaña "Todos"; en "Abiertos"/"Cerrados" todos los
// ítems ya comparten el mismo rango 0-2 o 3, así que el orden dentro de esa
// sub-pestaña no cambia por esto).
function welfareUrgencyRank(c) {
  if (c.status === 'cerrado') return 3;
  if (!c.nextReviewDate) return 2;
  return c.nextReviewDate < toISODate(new Date()) ? 0 : 1;
}
function sortWelfareByUrgency(items) {
  return [...items].sort((a, b) => {
    const ra = welfareUrgencyRank(a);
    const rb = welfareUrgencyRank(b);
    if (ra !== rb) return ra - rb;
    if (ra === 0 || ra === 1) return (a.nextReviewDate || '').localeCompare(b.nextReviewDate || '');
...
async function renderWelfareView() {
  ...
  container.innerHTML = `
    ...
    <div class="subtabs">
      ...
      <button class="subtab-btn ${state.welfareSubtab === 'resumen' ? 'active' : ''}" data-tab="resumen">📊 Resumen</button>
    </div>
  ...
  if (subtab === 'resumen') { renderWelfareSummary(content, cases); return; }
  const subtabItems = subtab === 'todos'
    ? cases
    : subtab === 'cerrados'
  ...
  content.innerHTML = `
    <div class="section-header" style="margin-top:0; flex-wrap:wrap; gap:10px;">
      <input type="text" id="wf-search" placeholder="Buscar por nombre…" value="${esc(state.welfareSearch || '')}" style="max-width:260px;" />
      <button class="btn btn-primary" id="wf-new">+ Nuevo caso</button>
    </div>
    <div style="display:flex; flex-wrap:wrap; gap:6px; margin:0 0 10px;">
      <button type="button" class="status-pill status-blue ministracion-filter-chip ${(state.welfareCategoryFilter || []).length ? '' : 'ministracion-filter-active'}" data-cat="" style="cursor:pointer; border:none; font:inherit; font-weight:700; text-transform:uppercase; font-size:11px; letter-spacing:.3px;">Todas</button>
      ${WELFARE_CATEGORIES.map((cat) => `
        <button type="button" class="status-pill status-gray ministracion-filter-chip wf-cat-chip ${(state.welfareCategoryFilter || []).includes(cat) ? 'ministracion-filter-active' : ''}" data-cat="${cat}" style="cursor:pointer; border:none; font:inherit; font-weight:700; font-size:11px;">${WELFARE_CATEGORY_LABELS[cat]}</button>
      `).join('')}
    </div>
    <p id="wf-count" style="font-size:12.5px; color:var(--ink-soft); margin:-2px 0 10px;"></p>
    <div class="card-list" id="wf-list"></div>
  `;
  const listEl = document.getElementById('wf-list');
  const countEl = document.getElementById('wf-count');
  // Igual que el buscador de Usuarios (renderAdminUsers): solo se
  // reconstruyen la lista y el contador en cada tecla — nunca el <input>
  // mismo — para que el foco no se pierda mientras se escribe.
  const renderRows = () => {
    const search = normalizeSearchText(state.welfareSearch || '');
    let items = subtabItems.filter((c) => welfareMatchesSearch(c, search) && welfareMatchesCategoryFilter(c, state.welfareCategoryFilter || []));
    items = sortWelfareByUrgency(items);
    listEl.innerHTML = items.length
      ? items.map(welfareCaseCardHtml).join('')
      : emptyStateHtml(subtabItems.length ? 'Nadie coincide con esa búsqueda o filtro' : 'Sin casos registrados en esta vista', { id: 'wf-empty-new', label: '+ Agregar el primero' }, subtabItems.length ? '🔍' : '🤝');
    countEl.textContent = (search || (state.welfareCategoryFilter || []).length) ? `${items.length} de ${subtabItems.length} casos` : `${subtabItems.length} caso${subtabItems.length === 1 ? '' : 's'}`;
    wireEmptyStateCta('wf-empty-new', () => openWelfareCaseModal(null, cases));
    listEl.querySelectorAll('.welfare-case-card').forEach((card) => {
      card.addEventListener('click', () => {
        const c = cases.find((x) => x.id === Number(card.dataset.id));
        if (c) openWelfareCaseDetailModal(c);
      });
    });
    // Acción rápida (Punto pedido explícitamente): revisar sin tener que
    // abrir primero el detalle completo del caso.
    listEl.querySelectorAll('[data-quick-review]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const c = cases.find((x) => x.id === Number(btn.dataset.quickReview));
        if (c) openWelfareQuickReviewModal(c);
      });
    });
  };
  renderRows();
  document.getElementById('wf-new').addEventListener('click', () => openWelfareCaseModal(null, cases));
  document.getElementById('wf-search').addEventListener('input', (e) => { state.welfareSearch = e.target.value; renderRows(); });
  content.querySelectorAll('[data-cat]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const cat = btn.dataset.cat;
      if (!cat) state.welfareCategoryFilter = [];
      else {
        const cur = state.welfareCategoryFilter || [];
        state.welfareCategoryFilter = cur.includes(cat) ? cur.filter((x) => x !== cat) : [...cur, cat];
      }
      content.querySelectorAll('[data-cat]').forEach((b) => {
        const active = b.dataset.cat ? (state.welfareCategoryFilter || []).includes(b.dataset.cat) : !(state.welfareCategoryFilter || []).length;
        b.classList.toggle('ministracion-filter-active', active);
      });
      renderRows();
    });
  });
...
function welfareCaseCardHtml(c) {
  ...
  return `
    <div class="list-card welfare-case-card" data-id="${c.id}" style="cursor:pointer;">
      <div class="lc-main">
        <div class="lc-title">${esc(c.memberName)} <span style="font-weight:400; font-size:12.5px; color:var(--ink-soft);">— ${esc(welfareCategoriesLabel(c))}</span></div>
...
// Modal liviano para la acción rápida "✓ Revisar" desde la tarjeta (Punto
// pedido explícitamente) — mismo formulario que ya existía dentro del
// detalle completo del caso (welfareAidSectionHtml), pero sin tener que
// abrir todo el detalle primero. Reutiliza el mismo endpoint /review.
function openWelfareQuickReviewModal(c) {
  const modalRoot = document.getElementById('modal-root');
  modalRoot.innerHTML = `
    <div class="modal-backdrop" id="wfqr-modal-backdrop">
      <div class="modal">
        <div class="modal-header"><h3>Revisar: ${esc(c.memberName)}</h3><button class="modal-close" id="wfqr-modal-close">×</button></div>
        <div class="modal-body">
          <div class="hint-box" style="margin-top:0;">${esc(welfareCategoriesLabel(c))} · Próxima evaluación: ${esc(fmtDateHuman(c.nextReviewDate))}</div>
          <form id="wfqr-form">
            <div class="field"><label>¿Cómo sigue esta persona/familia?</label>
              <select name="decision" id="wfqr-decision">
                <option value="extender">Se extiende la ayuda un mes más</option>
                <option value="solucionado">Se da por solucionado</option>
              </select>
            </div>
            <div class="field"><label>Notas (opcional)</label><textarea name="notes" rows="2"></textarea></div>
            ${welfareClosureChecklistHtml('wfqr')}
          </form>
        </div>
        <div class="modal-footer">
          <div></div>
          <div style="display:flex; gap:8px;">
            <button class="btn btn-secondary" id="wfqr-cancel">Cancelar</button>
            <button class="btn btn-primary" id="wfqr-save">Registrar evaluación</button>
          </div>
        </div>
      </div>
    </div>`;
  document.getElementById('wfqr-modal-close').addEventListener('click', closeModal);
  document.getElementById('wfqr-cancel').addEventListener('click', closeModal);
  ...
  document.getElementById('wfqr-save').addEventListener('click', withSavingState(document.getElementById('wfqr-save'), async () => {
    ...
    const fd = new FormData(form);
    const body = Object.fromEntries(fd.entries());
    if (body.decision === 'solucionado') body.closureChecklist = collectWelfareClosureChecklist(form);
    try {
      await api(`/welfare-cases/${c.id}/review`, { method: 'POST', body });
      closeModal();
...
// gráfico de casos por categoría, más la tasa de reapertura). Reutiliza la
// misma lista `cases` que ya se cargó para las otras sub-pestañas — sin
// pedir nada nuevo al servidor.
function renderWelfareSummary(content, cases) {
  const abiertos = cases.filter((c) => c.status === 'abierto').length;
  const enSeguimiento = cases.filter((c) => c.status === 'en_seguimiento').length;
  const thisYear = String(new Date().getFullYear());
  const cerradosEsteAno = cases.filter((c) => c.status === 'cerrado' && c.closedAt && c.closedAt.slice(0, 4) === thisYear).length;
  const ayudaUnicaVezActiva = cases.filter((c) => c.status !== 'cerrado' && c.aidType === 'unica_vez').length;
  const ayudaPeriodoActiva = cases.filter((c) => c.status !== 'cerrado' && c.aidType === 'periodo').length;

  // Tasa de reapertura (Punto pedido explícitamente): de los casos que
  // estuvieron cerrados alguna vez (lo estén ahora o hayan sido reabiertos
  // después), cuántos se volvieron a abrir.
  const everClosed = cases.filter((c) => c.status === 'cerrado' || (c.reopenLog && c.reopenLog.length) || (c.closures && c.closures.length));
  const reopened = cases.filter((c) => c.reopenLog && c.reopenLog.length);
  const reopenRate = everClosed.length ? Math.round((reopened.length / everClosed.length) * 100) : null;

  const categoryRows = WELFARE_CATEGORIES.map((cat) => ({
    label: WELFARE_CATEGORY_LABELS[cat],
    value: cases.filter((c) => welfareCaseCategories(c).includes(cat)).length,
    color: WELFARE_CATEGORY_COLORS[cat],
  }));

  content.innerHTML = `
    <div class="stats-cards" style="margin-bottom:16px;">
      <div class="stat-card"><div style="font-size:11px; color:var(--ink-soft); text-transform:uppercase;">Abiertos</div><div style="font-size:22px; font-weight:700;">${abiertos}</div></div>
      <div class="stat-card"><div style="font-size:11px; color:var(--ink-soft); text-transform:uppercase;">En seguimiento</div><div style="font-size:22px; font-weight:700;">${enSeguimiento}</div></div>
      <div class="stat-card"><div style="font-size:11px; color:var(--ink-soft); text-transform:uppercase;">Cerrados ${esc(thisYear)}</div><div style="font-size:22px; font-weight:700;">${cerradosEsteAno}</div></div>
      <div class="stat-card"><div style="font-size:11px; color:var(--ink-soft); text-transform:uppercase;">Ayuda activa · única vez</div><div style="font-size:22px; font-weight:700;">${ayudaUnicaVezActiva}</div></div>
      <div class="stat-card"><div style="font-size:11px; color:var(--ink-soft); text-transform:uppercase;">Ayuda activa · por período</div><div style="font-size:22px; font-weight:700;">${ayudaPeriodoActiva}</div></div>
    </div>
    ${reopenRate !== null ? `<div class="hint-box" style="margin-top:0; margin-bottom:16px;">🔁 Tasa de reapertura: ${reopened.length} de ${everClosed.length} caso${everClosed.length === 1 ? '' : 's'} que se cerró alguna vez volvió a abrirse (${reopenRate}%).</div>` : ''}
    <div style="font-weight:600; font-size:13.5px; color:var(--celeste-darker); margin-bottom:8px;">📊 Casos por categoría <span style="font-weight:400; font-size:12px; color:var(--ink-soft);">(todos los casos registrados; uno puede contar en más de una categoría)</span></div>
    ${cases.length ? wgTwoBarHtml(categoryRows) : emptyStateHtml('Todavía no hay casos registrados', null, '🤝', true)}
  `;
}

function openWelfareCaseModal(existing, allCases) {
  const isEdit = !!existing;
  const existingCategories = existing ? welfareCaseCategories(existing) : [];
  const modalRoot = document.getElementById('modal-root');
  modalRoot.innerHTML = `
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <form id="wf-form">
            ...
            <div class="field"><label>Categoría <span style="font-weight:400; color:var(--ink-soft);">(puedes marcar más de una)</span></label>
              <div style="display:flex; flex-wrap:wrap; gap:12px; margin-top:4px;">
                ${WELFARE_CATEGORIES.map((cat) => `
                  <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; cursor:pointer;">
                    <input type="checkbox" name="categories" value="${cat}" ${(isEdit ? existingCategories.includes(cat) : cat === WELFARE_CATEGORIES[0]) ? 'checked' : ''} />
                    ${WELFARE_CATEGORY_LABELS[cat]}
                  </label>`).join('')}
              </div>
            </div>
  ...
  // El checklist de cierre solo aplica cuando este cambio de estado hace
  // que el caso PASE a estar cerrado (no al editar un caso que ya estaba
  // cerrado sin tocar su estado) — mismo criterio que valida el servidor.
  const statusSel = document.getElementById('wf-status');
  if (statusSel) statusSel.addEventListener('change', () => {
    toggleWelfareChecklist('wf', statusSel.value === 'cerrado' && existing.status !== 'cerrado');
  });
  ...
  document.getElementById('wf-save').addEventListener('click', withSavingState(document.getElementById('wf-save'), async () => {
    ...
    if (!body.categories.length) {
      document.getElementById('wf-error').innerHTML = `<div class="error-msg">Marca al menos una categoría</div>`;
      return;
    }
    if (isEdit && body.status === 'cerrado' && existing.status !== 'cerrado') {
      body.closureChecklist = collectWelfareClosureChecklist(form);
    }
    // Punto pedido explícitamente: avisar si ya existe un caso ABIERTO con
    // el mismo nombre, para no duplicar por accidente el registro de la
    // misma persona/familia — solo al crear, nunca al editar (ahí el caso
    // ya es el mismo por definición).
    if (!isEdit && Array.isArray(allCases)) {
      const normalized = normalizeSearchText(body.memberName);
      const dup = allCases.find((c) => c.status !== 'cerrado' && normalizeSearchText(c.memberName) === normalized);
      if (dup) {
        const ok = await confirmModal(
          `Ya existe un caso abierto para "${dup.memberName}" (${welfareCategoriesLabel(dup)} · ${WELFARE_STATUS_LABELS[dup.status]}). ¿Quieres crear igual un caso nuevo?`,
          { title: 'Posible caso duplicado', confirmText: 'Crear de todos modos' },
        );
        if (!ok) return;
      }
    }
...
function openWelfareCaseDetailModal(c) {
  ...
  modalRoot.innerHTML = `
    <div class="modal-backdrop" id="wfd-modal-backdrop">
      <div class="modal" style="max-width:560px;">
        ...
        <div class="modal-body">
          <div class="hint-box" style="margin-top:0;">${esc(welfareCategoriesLabel(c))} · <span class="status-pill ${WELFARE_STATUS_PILL[c.status] || 'status-gray'}">${WELFARE_STATUS_LABELS[c.status] || c.status}</span>${(c.reopenLog && c.reopenLog.length) ? ` · <span class="status-pill status-amber">🔁 Reabierto ${c.reopenLog.length} ${c.reopenLog.length === 1 ? 'vez' : 'veces'}</span>` : ''}</div>
  ...
  // con su checklist de cierre — ver welfareClosureChecklistHtml).
