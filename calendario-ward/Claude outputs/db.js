...
export function load() {
  ...
  for (const c of data.welfareCases) {
    ...
    // migración: "categoría" pasó de un valor único a `categories` (arreglo)
    // — pedido explícito: "que se pueda seleccionar más de 1 categoría, a
    // veces las ayudas son esas 3 categorías juntas" — ver routes/welfare.js.
    if (!Array.isArray(c.categories)) { c.categories = c.category ? [c.category] : ['otro']; migrated = true; }
    if (c.category !== undefined) { delete c.category; migrated = true; }
    // migración: historial de cierres (con checklist) y de reaperturas —
    // pedido explícito de un checklist antes de cerrar un caso y de poder
    // ver la tasa de reapertura — ver closeWelfareCase/reopenWelfareCase.
